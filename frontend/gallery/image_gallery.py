...
class ImageGallery:
    ...
    def fetch_files(self):
        ...
        return (
            self.all_files[(self.page - 1) * 16 : self.page * 16],
            self.all_filenames[(self.page - 1) * 16 : self.page * 16],
        )
    ...
    def create_gallery(self):
        ...
        if self.gallery_type == "expander":
            if self.label == None:
                self.label = ""
                self.container_or_expander = st.expander(
                    label=self.label, expanded=self.expanded
                )
            else:
                self.container_or_expander = st.expander(
                    label=self.label, expanded=self.expanded
            ...
            for img in self.gallery_files:
                with self.all_columns[self.col_idx]:
                    if self.show_filename == True:
                        image = Image.open(img).resize((800, 500))
                        st.image(
                            img,
                            caption=self.gallery_filenames[self.filename_idx],
