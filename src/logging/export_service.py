...
class export_service:
    ...
    def save_all_results(self, arr: np.ndarray, columns: list, sub_folder_path: str, path_to_save: str):
    ...
    def store_model(self, model, modelName: str):
        ...
        now = datetime.now()
        date_time_as_str = str(now.day) + str(now.month) + str(now.year) + "_" + str(now.hour) + str(now.minute)
        model_scripted.save(os.path.join(self.folder_path, modelName + date_time_as_str + '.pt'))  # Save
